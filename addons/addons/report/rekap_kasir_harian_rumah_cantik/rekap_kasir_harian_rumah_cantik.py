	...
	for date in [
		...
		if items.get(date):
			...
			for item in items[date]["items"]:
				pasien_amount(item, result, "item_group", "Rumah Cantik", 'amount_produk', pasien_produk, "Rumah Cantik")
				pasien_amount(item, result, "item_group", "Layanan Rumah Cantik", 'amount_treatment', pasien_treatment, "Rumah Cantik")

			result['pasien_baru'] = len([key for key, value in items[date]["customer"].items() if value == "Rumah Cantik"]) 
		...
		if invoice_payment.get(date):
			for item in invoice_payment[date]["payment"]:
				...
				pasien_amount(item, result, "mode_of_payment", "QRIS BRI", 'qris_bri')
				...
				pasien_amount(item, result, "mode_of_payment", "EDC Debit", 'edc_debit')
